...
local function AmbulanceInjuriedPerson()
    local coords = GetEntityCoords(cache.ped)
...
exports('AmbulanceInjuriedPerson', AmbulanceInjuriedPerson)

local function PDoctorInjuriedPerson()
    local coords = GetEntityCoords(cache.ped)
...
exports('PDoctorInjuriedPerson', PDoctorInjuriedPerson)

local function POLICEInjuriedPerson()
    local coords = GetEntityCoords(cache.ped)
...
exports('POLICEInjuriedPerson', POLICEInjuriedPerson)
...
local function PhoneCall(message, anonymous, job, type)
    ...
    if IsCallAllowed(message) then
        ...
        local dispatchData = {
            ...
            codeName = type == '311' and '311call' or '911call'or '411' and '411call',
            code = type,
            icon = 'fas fa-phone',
            priority = 2,
            coords = coords,
...
RegisterNetEvent('ps-dispatch:client:sendEmergencyMsg', function(data, type, anonymous)
    local jobs = { ['911'] = { 'leo','bcso' }, ['311'] = { 'ems' }, ['411'] = { 'pdoctor' } }
...
local function DrugDealer()
    local coords = GetEntityCoords(cache.ped)

    local dispatchData = {
        message = locale('drugdealer'), -- add this into your locale
        codeName = 'DrugDealer', -- this should be the same as in config.lua
        code = '10-35',
        icon = 'fas fa-car-burst',
        priority = 2,
        coords = coords,
...
exports('DrugDealer', DrugDealer)

local function voltheist()
    local coords = GetEntityCoords(cache.ped)

    local dispatchData = {
        message = locale('voltheist'),
        codeName = 'voltheist',
        code = '10-90',
        icon = 'fas fa-truck-field',
        priority = 2,
...
exports('voltheist', voltheist)

local function oillheist()
    local coords = GetEntityCoords(cache.ped)

    local dispatchData = {
        message = locale('oillheist'),
        codeName = 'oillheist',
        code = '10-90',
        icon = 'fas fa-truck-field',
        priority = 2,
